function [targets_pred, sigma]= NAX(dataset, hidden_neurons, act_fun, reg_param, first_year, last_year, test_year, flag)
...
net = narxnet(0,1,hidden_neurons, 'closed');%0: time lag on input
                                            ...
                                            %closed loop network
net.inputs{1}.size = 20;            %10 inputs x(t), 10 inputs x(t-1)
...
targets_train = mat2cell(targets_train, 1, ones(1,lasty_pos - 1));
...
err = train_pred - cell2mat(targets_train);
...
if (flag)
    plot([1:length(targets_pred)],targets_pred)
    hold on
    plot([1:length(targets_pred)],targets_test)
end
